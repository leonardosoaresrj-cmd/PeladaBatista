 ...
 *
 * ARQUITETURA:
 *   1. Modal abre → chama pix-criar → MP gera cobrança dinâmica real
 *   2. Exibe QR Code real (base64 do MP) + copia-e-cola
 *   3. Polling a cada 5s em pix-status → detecta pagamento aprovado
 *   4. Quando aprovado: chama onConfirmarPagamentoTotal → grava no Supabase
 *   5. Em paralelo: pix-webhook do MP também grava (garantia dupla)
 *
 * FALLBACK: se pix-criar falhar → exibe PIX estático com botão de confirmação manual
 */
...
import {
  ...
  CheckCircle2, DollarSign, AlertCircle, ShieldCheck, Info, Zap
} from 'lucide-react';
...
export default function CheckoutPixModal({
  ...
  }, [debitos, onConfirmarPagamentoTotal, pararTudo]);
  ...
  // ── Criar cobrança real no MP via Edge Function ────────────────────────────
  const criarCobrancaMP = async () => {
    try {
      ...
      const info = `HTTP ${resp.status} | ${JSON.stringify(data).substring(0, 100)}`;
      setDebugInfo(info);
      console.log('[pix-criar] resposta:', info);
      ...
      if (resp.ok && data.success && data.qrCodeText) {
        ...
        console.warn('[pix-criar] fallback:', data?.error);
        ativarFallback(data?.error);
      }
    } catch (e: any) {
      console.error('[pix-criar] erro de rede:', e?.message);
  ...
  const iniciarPolling = (paymentId: string) => {
    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        if (resp.ok) {
          const data = await resp.json();
          console.log('[pix-status] status:', data.status);
          if (data.pago) {
            confirmarPagamento();
          }
        }
  ...
  // QR Code: base64 real do MP ou gerador externo
        ...
        >
          ...
          {step === 'gerando' && (
            <div className="p-12 flex flex-col items-center justify-center space-y-4">
              ...
              {debugInfo && (
                <p className="text-[10px] text-yellow-400 font-mono text-center max-w-[320px] break-all">{debugInfo}</p>
              )}
            </div>
          )}
            ...
            <div className="p-6 space-y-5 overflow-y-auto flex-1">
              ...
              <div className="bg-emerald-950/50 rounded-2xl p-4 border border-white/5 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[9.5px] text-emerald-300 mt-1">
              ...
              <div className="flex flex-col items-center justify-center space-y-3 bg-white/5 border border-white/5 p-5 rounded-2xl">
                ...
                <div className="text-center">
                  ...
                  {!modoFallback && (
                    <p className="text-[9px] text-teal-300 mt-1 flex items-center gap-1 justify-center">
              ...
              {modoFallback ? (
                <div className="bg-amber-950/40 border border-amber-500/30 rounded-2xl p-4 space-y-3">
                  ...
                  <button onClick={confirmarPagamento}
                    className="w-full bg-amber-500 text-black font-bold text-[11px] py-2.5 rounded-xl cursor-pointer uppercase tracking-wider">
                    ✅ Já Paguei — Confirmar Quitação
                  </button>
                </div>
              ) : (
                <div className="bg-teal-950/50 border border-teal-500/30 rounded-2xl p-4 space-y-3">
                  <div className="flex gap-3 text-xs">
                    <ShieldCheck className="w-5 h-5 text-teal-400 shrink-0 mt-0.5" />
                    <div>
                      <h4 className="font-bold text-teal-300 uppercase text-[9px] tracking-wider">MERCADO PAGO — CONFIRMAÇÃO AUTOMÁTICA</h4>
                      <p className="text-[10px] text-teal-100 mt-0.5 leading-relaxed">
                        O sistema verifica a cada 5s. Se já pagou e a tela não atualizou automaticamente, clique abaixo.
                      </p>
                    </div>
                  </div>
                  <button onClick={confirmarPagamento}
                    className="w-full bg-teal-500/20 border border-teal-500/40 text-teal-300 font-bold text-[11px] py-2 rounded-xl cursor-pointer uppercase tracking-wider">
                    ✅ Já Paguei — Confirmar Quitação
                  </button>
                </div>
              )}
              ...
              {/* Debug info (dev) */}
              {debugInfo && (
                <p className="text-[9px] text-yellow-500/60 font-mono break-all text-center">{debugInfo}</p>
              )}
            </div>
          )}
            ...
            <div className="p-10 text-center flex flex-col items-center justify-center space-y-5 py-12 overflow-y-auto flex-1">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-teal-100 max-w-[340px] leading-relaxed">
                  R$ {valorTotal.toFixed(2)} quitado com sucesso.
