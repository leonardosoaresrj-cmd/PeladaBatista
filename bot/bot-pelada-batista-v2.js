...
app.post('/teste', async (req, res) => {
  ...
  try {
    ...
    if (ehErroCriptografia(err)) {
      // Conexão zumbi — reinicia sem esperar
      isConnected = false;
      setTimeout(conectarWhatsApp, 1000);
      return res.status(503).json({
        error: 'Conexão WhatsApp instável — reconectando automaticamente. Tente novamente em 10s.',
        reconnecting: true,
      });
    }
...
async function enviarParaGrupo(texto) {
  ...
  try {
    ...
    if (ehErroCriptografia(err)) {
      console.warn('⚠️  Erro de criptografia em enviarParaGrupo — forçando reconexão...');
      isConnected = false;
      setTimeout(conectarWhatsApp, 1000);
    }
    throw err;
...
async function conectarWhatsApp() {
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'close') {
      ...
      // Erro de criptografia Signal → limpa sessão corrompida antes de reconectar
      if (errMsg.includes('Bad MAC') || errMsg.includes('MessageCounterError') || errMsg.includes('Key used already')) {
        console.warn('🔑 Erro de criptografia Signal detectado — limpando sessão local...');
        try { fs.rmSync(CONFIG.SESSION_LOCAL_PATH, { recursive: true, force: true }); } catch {}
        fs.mkdirSync(CONFIG.SESSION_LOCAL_PATH, { recursive: true });
      }
