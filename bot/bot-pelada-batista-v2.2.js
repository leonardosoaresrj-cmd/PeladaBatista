...
// -- GET /grupos — lista todos os grupos do numero conectado ------------------
app.get('/grupos', async (req, res) => {
  ...
  try {
    // Busca todos os grupos que o numero participa
    ...
    console.error('[ERR] /grupos:', err.message);
    res.status(500).send(`
...
app.post('/teste', async (req, res) => {
  if (req.headers['x-webhook-secret'] !== CONFIG.WEBHOOK_SECRET) {
    console.warn('[WARN] /teste: secret invalido');
  ...
  try {
    ...
    console.log(`[SEND] [/teste] -> ${jid} | ${mensagem.substring(0, 60).replace(/\n/g, ' ')}`);
    ...
    console.error('[ERR] /teste:', err.message);
...
app.post('/webhook', async (req, res) => {
  ...
  console.log(`[RECV] [/webhook] ${type} em "${table}"`);

  try {
    await processarEventoWebhook(type, table, record, old_record);
    res.json({ success: true });
  } catch (err) {
    console.error('[ERR] webhook:', err.message);
    res.status(500).json({ error: err.message });
  }
});

// -----------------------------------------------------------------------------
// SESSÃO — Estrategia hibrida:
//   1ª tentativa: tabela bot_session no Supabase (JSON consolidado, sem ENOENT)
//   2ª tentativa: Supabase Storage (compatibilidade com sessoes existentes)
// -----------------------------------------------------------------------------
...
async function salvarSessao() {
  ...
  try {
    ...
    // Serializa todos os arquivos existentes em um unico objeto JSON
    const sessaoObj = {};
    ...
    if (error) {
      ...
      console.log(`[SAVE] Sessao salva (${Object.keys(sessaoObj).length} arquivos -> tabela bot_session)`);
    }
  } catch (err) {
    console.error('[ERR] Erro ao salvar sessao:', err.message);
  }
}
...
async function salvarSessaoStorage(sessaoObj) {
  ...
  if (salvos > 0) console.log(`[SAVE] Sessao salva no Storage (${salvos} arquivos)`);
}

async function baixarSessao() {
  console.log('[RECV] Restaurando sessao...');
  ...
  try {
    ...
    if (!error && data?.session_data) {
      ...
      if (nomes.length > 0) {
        ...
        console.log(`[OK] Sessao restaurada da tabela bot_session (${nomes.length} arquivos)`);
        return true;
      }
    }
  } catch (err) {
    console.warn('[WARN] bot_session nao disponivel, tentando Storage...', err.message);
  }

  // Tentativa 2: Supabase Storage (sessoes antigas com 100 arquivos)
  try {
    // O storage pode paginar — busca ate 200 arquivos
    const { data: arquivos } = await supabase.storage
      .from(CONFIG.SUPABASE_BUCKET)
      .list('session/', { limit: 200 });

    if (!arquivos?.length) {
      console.log('ℹ️  Sem sessao salva — escaneie o QR Code em /qr');
      return false;
    }
    ...
    if (restaurados > 0) {
      console.log(`[OK] Sessao restaurada do Storage (${restaurados} arquivos)`);
      // Migra para a tabela bot_session para proximas reinicializacoes
      await salvarSessao();
      return true;
    }
  } catch (err) {
    console.error('[ERR] Erro ao restaurar sessao do Storage:', err.message);
  }
...
async function limparSessao() {
  ...
  // Limpa tabela bot_session (todos os registros, nao so 'main')
  try {
    await supabase.from('bot_session').delete().neq('id', '___never___');
    console.log('[DEL] bot_session limpa');
  } catch (e) { console.warn('[WARN] Erro limpando bot_session:', e.message); }
  ...
  try {
    ...
    console.log('[DEL] Storage limpo');
  } catch (e) { console.warn('[WARN] Erro limpando Storage:', e.message); }

  console.log('[DEL] Sessao limpa');
}
...
function iniciarCronDiario() {
  ...
  cronInterval = setInterval(async () => {
    ...
    // -- 04h: limpeza seletiva de sessao (previne acumulacao de arquivos Signal)
    if (hora === 4 && agora.getMinutes() < 2) {
      await limparSessaoSeletiva();
    }

    // -- 08h: REGRA 1 — abertura de renovacao de mensalidade
    if (hora === 8 && ehPrimeiroDiaPosUltimoSabadoMesAnterior(agora)) {
      console.log('[CRON] REGRA 1 — disparando abertura de renovacao...');
  ...
  console.log('[CRON] Cron diario ativo (limpeza as 04h | renovacao as 08h)');
}


// -----------------------------------------------------------------------------
// LIMPEZA SELETIVA DE SESSÃO — roda diariamente as 04h
// Remove arquivos descartaveis (session-*, pre-keys-*, app-state-sync-version-*)
// mantendo os essenciais (creds.json, app-state-sync-key-*, sender-key-mem-*)
// Sem isso, o Storage acumula 500+ arquivos levando a corrupcao do estado Signal
// (signedKeyId reaproveitado -> "mensagem enviada mas nao recebida" no WhatsApp)
// -----------------------------------------------------------------------------
...
async function limparSessaoSeletiva() {
  console.log('[CLEAN] [CRON] Iniciando limpeza seletiva de sessao...');
  try {
    ...
    while (true) {
      ...
      if (!descartaveis.length) {
        console.log(`[CLEAN] [CRON] Sessao ja limpa (${nomes.length} arquivos, nenhum descartavel)`);
        break;
      }

      // 2. Criar novo objeto so com arquivos essenciais
      const sessaoLimpa = {};
      for (const nome of essenciais) {
        sessaoLimpa[nome] = sessaoObj[nome];
      }

      // 3. Salvar sessao limpa de volta na tabela
      ...
      // 4. Limpar tambem os arquivos locais descartaveis
      if (fs.existsSync(CONFIG.SESSION_LOCAL_PATH)) {
      ...
      console.log(
        `[CLEAN] [CRON] Limpeza concluida: ${nomes.length} -> ${essenciais.length} arquivos ` +
        `(removidos: ${descartaveis.length} | mantidos: ${essenciais.join(', ').substring(0, 80)}...)`
      );
      break;
    }

    // 5. Limpar Storage tambem (fallback storage)
    try {
      let offset = 0;
      while (true) {
        const { data: arqs } = await supabase.storage
        ...
        if (paraRemover.length) {
          ...
          console.log(`[CLEAN] [CRON] Storage: removidos ${paraRemover.length} arquivos descartaveis`);
        }
        if (arqs.length < 200) break;
        offset += 200;
      }
    } catch (e) {
      console.warn('[WARN] Erro limpando Storage (nao critico):', e.message);
    }

  } catch (err) {
    console.error('[ERR] Erro na limpeza seletiva de sessao:', err.message);
  }
}
...
async function dispararAberturaMensalidade(hoje) {
  try {
    if (!isConnected) { console.warn('[WARN] [REGRA1] WA nao conectado'); return; }
    ...
    console.log(`[OK] [REGRA1] Abertura de renovacao enviada para ${mesRef}`);
  } catch (err) {
    console.error('[ERR] [REGRA1]:', err.message);
  }
}
...
async function processarEventoWebhook(tipo, tabela, rec, recAntigo) {
  ...
  if (tabela === 'pagamentos') {
    ...
    if ((tipo === 'INSERT' && rec.status === 'pago') || (tipo === 'UPDATE' && ficouPago)) {
      ...
      setTimeout(async () => {
        ...
        } catch (e) { console.error('[ERR] Lista mensalidade:', e.message); }
      }, 3000);
    }
  }
  ...
  // Notificacao extra para o Administrador: Novo Jogador cadastrado aguardando aprovacao
  if (tabela === 'jogadores' && tipo === 'INSERT' && rec.status === 'pendente_aprovacao') {
    ...
    setTimeout(async () => {
      try {
        ...
        console.log(`[OK] [NOTIFICAÇÃO ADMIN] Novo cadastro de ${rec.nome} enviado para o administrador.`);
      } catch (err) {
        console.error('[ERR] Erro ao notificar admin sobre novo jogador:', err.message);
      }
    }, 1000);
  }

  // Notificacao extra para o Administrador: Novo Pagamento manual aguardando aprovacao
  if (tabela === 'pagamentos') {
    ...
    if ((tipo === 'INSERT' && rec.status === 'pendente_confirmacao') || (tipo === 'UPDATE' && ficouPendenteConfirmacao)) {
      setTimeout(async () => {
        try {
          ...
          console.log(`[OK] [NOTIFICAÇÃO ADMIN] Pagamento de ${nomeAtleta} enviado para o administrador.`);
        } catch (err) {
          console.error('[ERR] Erro ao notificar admin sobre pagamento:', err.message);
        }
      }, 1000);
    }
  }
...
async function gerarListaCompletaPartida(partidaId) {
  try {
    ...
    console.error('[ERR] gerarListaCompleta:', err.message);
    return null;
  }
}
...
async function resolverJID(grupoId) {
  ...
  if (grupoId === 'admin') {
    if (sock && sock.user && sock.user.id) {
      ...
      console.log(`[TARGET] Envio para Administrador (Self): ${cleanJid}`);
      return cleanJid;
    }
  ...
  if (grupoId.includes('@s.whatsapp.net')) {
    ...
    // Preserva o hifen do ID do grupo (ex: 5521996134821-1396914476)
    const idLimpo = grupoId.replace(/[^0-9\-]/g, '').replace(/-+/g, '-').replace(/^-|-$/g, '');
    jid = `${idLimpo}@g.us`;
  }
  jidCache.set(grupoId, jid);
  console.log(`[OK] JID: ${grupoId} -> ${jid}`);
  return jid;
}
...
// Detecta se o erro e de criptografia Signal
...
async function enviarParaGrupo(texto) {
  ...
  try {
    ...
    if (ehErroCriptografia(err) && !limpandoSessao) {
      limpandoSessao = true;
      console.warn('[WARN] Erro de criptografia em enviarParaGrupo — limpando sessao e reconectando...');
      isConnected = false;
      limparSessao()
        .then(() => setTimeout(() => { limpandoSessao = false; conectarWhatsApp(); }, 2000))
        .catch(() => setTimeout(() => { limpandoSessao = false; conectarWhatsApp(); }, 2000));
...
function iniciarSelfPing() {
  ...
  pingInterval = setInterval(async () => {
    ...
    } catch { console.warn('[WARN] Self-ping falhou'); }
  }, CONFIG.PING_INTERVAL_MS);
  console.log(`[PING] Self-ping ativo (a cada ${CONFIG.PING_INTERVAL_MS / 1000}s)`);
}
...
async function conectarWhatsApp() {
  if (sock) {
    try {
      ...
      console.warn('[WARN] Erro ao limpar socket anterior (ignorado):', e.message);
    }
    sock = null;
  }
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'open') {
      ...
      if (sessaoCorrompidaDetectada) {
        ...
        console.warn('🔑 Sessao corrompida detectada logo apos conectar — limpando...');
        await limparSessao();
    ...
    if (connection === 'close') {
      ...
      console.warn(`[WARN] Conexao encerrada — codigo: ${code}`);

      if (errMsg.includes('Bad MAC') || errMsg.includes('MessageCounterError') || errMsg.includes('Key used already')) {
        console.warn('🔑 Bad MAC detectado — limpando sessao completa (local + Supabase)...');
        await limparSessao();
      }

      if (code === DisconnectReason.loggedOut) {
        console.error('🚪 Logout! Limpando sessao...');
        await limparSessao();
      ...
      console.log(`[SYNC] Reconectando em ${delay / 1000}s (#${reconnectAttempts})...`);
...
async function iniciar() {
  console.log('[START] Robo Pelada Batista v2.2 — iniciando...');
  console.log(`[NET] URL: ${CONFIG.SELF_URL}`);
  console.log(`[LOCK] Secret: ${CONFIG.WEBHOOK_SECRET ? '[OK]' : '[ERR] NÃO configurado!'}`);
  console.log('[INFO] Regras: REGRA1(cron+webhook) | REGRA2(webhook) | REGRA3(webhook)');

  app.listen(CONFIG.PORT, () => {
    console.log(`🌐 Servidor na porta ${CONFIG.PORT}`);
    console.log(`[QR] QR Code em: ${CONFIG.SELF_URL}/qr`);
  });
